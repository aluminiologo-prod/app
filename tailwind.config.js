/** @type {import('tailwindcss').Config} */
module.exports = {
  content: [
    './app/**/*.{js,jsx,ts,tsx}',
    './src/**/*.{js,jsx,ts,tsx}',
  ],
  presets: [require('nativewind/preset')],
  darkMode: 'class',
  theme: {
    extend: {
      colors: {
        primary: {
          50:  '#EBF1FF',
          100: '#D6E4FF',
          200: '#ADC8FF',
          300: '#84ADFF',
          400: '#5B91FF',
          500: '#3874FF',
          600: '#2D5DCC',
          700: '#224699',
          800: '#172F66',
          900: '#0B1833',
          DEFAULT: '#3874FF',
        },
        secondary: {
          50:  '#ECEDF0',
          100: '#D9DBE1',
          200: '#B3B7C3',
          300: '#8D93A5',
          400: '#676F87',
          500: '#31374A',
          600: '#272C3B',
          700: '#1D212C',
          800: '#13161D',
          900: '#0A0B0F',
          DEFAULT: '#31374A',
        },
        success:    { DEFAULT: '#25B003', foreground: '#FFFFFF' },
        warning:    { DEFAULT: '#E5780B', foreground: '#FFFFFF' },
        danger:     { DEFAULT: '#EC1F00', foreground: '#FFFFFF' },
        background: '#FFFFFF',
        foreground: '#11181C',
        content1:   '#FFFFFF',
        content2:   '#F4F4F5',
        content3:   '#E4E4E7',
        content4:   '#D4D4D8',
        border:     '#E4E4E7',
        brand: {
          orange:      '#E5780B',
          orangeDeep:  '#C86306',
          orangeSoft:  '#FDEBD4',
          cream:       '#FAF7F2',
          creamSoft:   '#F3EFE7',
          navy:        '#1A2A4A',
          navyMuted:   '#62728B',
          navyDark:    '#0C1B30',
          navyDarker:  '#0A1629',
          navySurface: '#223558',
          dotLight:    '#E8E1D3',
          dotDark:     '#1B2B48',
        },
      },
      fontFamily: {
        sans:     ['Inter_400Regular',         'sans-serif'],
        medium:   ['Inter_500Medium',          'sans-serif'],
        semibold: ['Inter_600SemiBold',        'sans-serif'],
        bold:     ['Inter_700Bold',            'sans-serif'],
        serif:         ['Fraunces_400Regular',         'serif'],
        'serif-semi':  ['Fraunces_600SemiBold',        'serif'],
        'serif-bold':  ['Fraunces_700Bold',            'serif'],
        'serif-italic':     ['Fraunces_400Regular_Italic', 'serif'],
        'serif-italic-semi': ['Fraunces_600SemiBold_Italic', 'serif'],
        'serif-italic-bold': ['Fraunces_700Bold_Italic',    'serif'],
      },
      borderRadius: {
        '2xl': '16px',
        '3xl': '24px',
      },
    },
  },
  plugins: [],
};
